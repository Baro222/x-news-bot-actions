"""
X 뉴스 자동 수집 및 텔레그램 발송 메인 스크립트 (v2)

개선사항:
1. ai_processor_v2 사용 (LibreTranslate 번역 + 참여도 점수)
2. telegram_sender_v2 사용 (각 주제별 1위 별도 포스팅)
3. 좋아요/조회수 기반 우선순위

실행: python3.11 main_v2.py
"""
...
if not root_logger.handlers:
    ...
    try:
        fh = logging.FileHandler(os.path.join(LOG_DIR, 'news_bot_v2.log'), encoding='utf-8')
...
def run_news_cycle_v2():
    """
    뉴스 수집 -> AI 처리 (v2) -> 텔레그램 발송 (v2) 전체 사이클 실행
    """
    
    start_time = datetime.now(KST)
    logger.info(f"===== 뉴스 수집 사이클 시작 (v2): {start_time.strftime('%Y-%m-%d %H:%M KST')} =====")
    ...
    try:
        ...
        if not tweets:
            ...
            from telegram_sender_v2 import _send_via_bot
            _send_via_bot(
        ...
        # 2단계: AI 분류 및 요약 (v2 - 번역 + 참여도 점수)
        logger.info("2단계: AI 분류, 번역, 참여도 점수 계산 시작...")
        from ai_processor_v2 import process_tweets, rank_and_select_news
        
        processed_tweets = process_tweets(tweets)
        ranked_news = rank_and_select_news(processed_tweets, max_per_category=10)
        ...
        # 3단계: 텔레그램 발송 (v2 - 각 주제별 1위 별도 포스팅)
        logger.info("3단계: 텔레그램 발송 시작 (v2 형식)...")
        from telegram_sender_v2 import send_news_report_v2
        success = send_news_report_v2(ranked_news)
        ...
        # 결과 저장 (디버깅용)
        result_file = f"{LOG_DIR}/last_result_v2.json"
        save_data = {
