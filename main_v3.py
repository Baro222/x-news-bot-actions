"""
X 뉴스 자동 수집 및 텔레그램 발송 메인 스크립트 (v3)
...
if not root_logger.handlers:
    ...
    try:
        fh = logging.FileHandler(os.path.join(LOG_DIR, 'news_bot_v3.log'), encoding='utf-8')
...
def run_news_cycle_v3():
    """뉴스 수집 → AI 처리 → 텔레그램 발송 전체 사이클 (v3)"""
    
    start_time = datetime.now(KST)
    logger.info(f"===== 뉴스 수집 사이클 시작 (v3): {start_time.strftime('%Y-%m-%d %H:%M KST')} =====")
    ...
    try:
        ...
        if not tweets:
            ...
            from telegram_sender_v3 import _send_via_bot
            _send_via_bot(
        ...
        from ai_processor_v3 import process_tweets, rank_and_select_news
        ...
        # 3단계: 텔레그램 발송 (v3 형식)
        logger.info("3단계: 텔레그램 발송 시작 (v3 형식)...")
        from telegram_sender_v3 import send_news_report_v3
        success = send_news_report_v3(ranked_news)
        ...
        # 결과 저장
        result_file = f"{LOG_DIR}/last_result_v3.json"
        save_data = {
