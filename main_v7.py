"""
X 뉴스 자동 수집 및 텔레그램 발송 메인 스크립트 (v7 - 최종)

실행: python3.11 main_v7.py
"""
...
if not root_logger.handlers:
    ...
    try:
        fh = logging.FileHandler(os.path.join(LOG_DIR, 'news_bot_v7.log'), encoding='utf-8')
...
def run_news_cycle_v7():
    """뉴스 수집 → AI 처리 → 텔레그램 발송 전체 사이클 (v7 최종)"""
    
    start_time = datetime.now(KST)
    logger.info(f"===== 뉴스 수집 사이클 시작 (v7): {start_time.strftime('%Y-%m-%d %H:%M KST')} =====")
    ...
    try:
        ...
        if not tweets:
            ...
            from telegram_sender_v7 import _send_via_bot
            _send_via_bot(
        ...
        # 3단계: 텔레그램 발송 (v7 최종 형식)
        logger.info("3단계: 텔레그램 발송 시작 (v7 형식: stockhubkr TOP 1 + saveticker 랭크)...")
        from telegram_sender_v7 import send_news_report_v7
        success = send_news_report_v7(ranked_news)
        ...
        # 결과 저장
        result_file = f"{LOG_DIR}/last_result_v7.json"
        save_data = {
