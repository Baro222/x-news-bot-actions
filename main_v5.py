"""
X 뉴스 자동 수집 및 텔레그램 발송 메인 스크립트 (v5)
...
4. 참여도 점수 기반 우선순위
5. wojaklisting/baroBTC 포스팅 형식
6. 랭킹 시스템
7. 팔로업 계정 리스트 생성

실행: python3.11 main_v5.py
"""
...
if not root_logger.handlers:
    ...
    try:
        fh = logging.FileHandler(os.path.join(LOG_DIR, 'news_bot_v5.log'), encoding='utf-8')
...
def run_news_cycle_v5():
    """뉴스 수집 → AI 처리 → 텔레그램 발송 전체 사이클 (v5)"""
    
    start_time = datetime.now(KST)
    logger.info(f"===== 뉴스 수집 사이클 시작 (v5): {start_time.strftime('%Y-%m-%d %H:%M KST')} =====")
    ...
    try:
        ...
        if not tweets:
            ...
            from telegram_sender_v5 import _send_via_bot
            _send_via_bot(
        ...
        # 3단계: 텔레그램 발송 (v5 형식: wojaklisting + baroBTC)
        logger.info("3단계: 텔레그램 발송 시작 (v5 형식)...")
        from telegram_sender_v5 import send_news_report_v5
        success = send_news_report_v5(ranked_news)
        ...
        # 결과 저장
        result_file = f"{LOG_DIR}/last_result_v5.json"
        save_data = {
