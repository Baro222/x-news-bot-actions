"""
X 뉴스 자동 수집 및 텔레그램 발송 메인 스크립트 (v6)

개선사항:
1. 73개 X 핸들 모니터링 (팔로업 정보 포함)
2. Nitter RSS 기반 트윗 수집
3. LibreTranslate 기반 한글 번역
4. 참여도 점수 기반 우선순위
5. wojaklisting 형식 - 큰 이슈/암호화폐/트럼프 뉴스
6. baroBTC 랭크 형식 - 카테고리별 랭크 포스팅
7. 랭킹 시스템
8. 팔로업 계정 리스트 생성

실행: python3.11 main_v6.py
"""
...
if not root_logger.handlers:
    ...
    try:
        fh = logging.FileHandler(os.path.join(LOG_DIR, 'news_bot_v6.log'), encoding='utf-8')
...
def save_followup_accounts_list() -> None:
    """팔로업 계정 리스트를 저장합니다."""
    from twitter_fetcher_v4 import get_account_followup_list
    
    followup_list = get_account_followup_list()
    
    # 마크다운 형식으로 저장
    md_content = "# X 뉴스봇 팔로업 계정 리스트\n\n"
    md_content += f"**총 73개 계정 팔로업 중**\n\n"
    
    for category in sorted(followup_list.keys()):
        accounts = followup_list[category]
        md_content += f"## {category} ({len(accounts)}개)\n\n"
        
        for acc in accounts:
            followers = acc.get('followers', 'medium')
            followers_emoji = {
                'very_high': '⭐⭐⭐',
                'high': '⭐⭐',
                'medium': '⭐',
            ...
            md_content += f"- {acc['handle']} {followers_emoji} - {acc['description']}\n"
        
        md_content += "\n"
    
    # 저장
    output_path = os.path.join(repo_root, 'FOLLOWUP_ACCOUNTS.md')
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(md_content)
    
    logger.info(f"팔로업 계정 리스트 저장 완료: {output_path}")
...
def run_news_cycle_v6():
    """뉴스 수집 → AI 처리 → 텔레그램 발송 전체 사이클 (v6)"""
    
    start_time = datetime.now(KST)
    logger.info(f"===== 뉴스 수집 사이클 시작 (v6): {start_time.strftime('%Y-%m-%d %H:%M KST')} =====")
    
    try:
        # 1단계: 트윗 수집
        logger.info("1단계: X 트윗 수집 시작 (73개 계정)...")
        from twitter_fetcher_v4 import fetch_all_tweets
        tweets = fetch_all_tweets()
        
        if not tweets:
            logger.warning("수집된 트윗이 없습니다. 사이클 종료.")
            from telegram_sender_v6 import _send_via_bot
            _send_via_bot(
        ...
        # 3단계: 텔레그램 발송 (v6 형식: wojaklisting + baroBTC 랭크)
        logger.info("3단계: 텔레그램 발송 시작 (v6 형식: wojaklisting + baroBTC 랭크)...")
        from telegram_sender_v6 import send_news_report_v6
        success = send_news_report_v6(ranked_news)
        ...
        # 4단계: 팔로업 계정 리스트 저장
        logger.info("4단계: 팔로업 계정 리스트 저장...")
        save_followup_accounts_list()
        
        # 결과 저장
        result_file = f"{LOG_DIR}/last_result_v6.json"
        save_data = {
