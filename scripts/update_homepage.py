...
# Parse mockData.ts for mockNews entries (title + summary)
text = MOCK.read_text(encoding='utf-8')
# Find the mockNews array content
m = re.search(r"export const mockNews:\s*NewsItem\[\] = \[(.*?)\];", text, re.S)
items = []
if m:
    arr = m.group(1)
    # split by '},' that likely separates objects
...
# Fallback: if parsing failed, use simple defaults
if not items:
    items = [
        {'title':'샘플 뉴스 없음','summary':'데이터를 불러올 수 없습니다.','source':'local'}
    ]

# Select top 5 based on order
selected = items[:5]
...
print('homepage updated with', len(selected), 'items')
